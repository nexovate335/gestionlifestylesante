...
def generate_unique_numero_facture():
    ...
    return f"HO-{random_number}"

class FactureCaisseManager(models.Manager):
...
class FactureCaisse(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = "Factures Caisses"
    ...
    @property
    def get_total(self):
        caisses = self.caisse_set.filter(deleted_at__isnull=True)
...
class FactureAvance(models.Model):
    facture = models.ForeignKey(FactureCaisse, on_delete=models.CASCADE, related_name="avances", verbose_name="Numéro de facture")
...
class CaisseManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(deleted_at__isnull=True)
...
class Caisse(models.Model):
    ...
    facture = models.ForeignKey(FactureCaisse, on_delete=models.PROTECT, verbose_name="Facture")
    montant = models.DecimalField(
...
class AutresDepensesManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(deleted_at__isnull=True)
...
class RapportJournalierCaisseManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(deleted_at__isnull=True)
...
class RapportJournalierCaisse(models.Model):
    ...
    objects = RapportJournalierCaisseManager()
    all_objects = models.Manager()

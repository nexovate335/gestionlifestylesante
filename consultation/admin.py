...
class ConsultationAdmin(BaseAdmin):
    ...
    fieldsets = (
        ...
        ("Antécédents", {
            'fields': (
                'atcd_socioeconomique',
                'atcd_chomage',
                'atcd_violence_domestique',
                'atcd_prison',
                'atcd_arret_ecole',
                'atcd_victime_guerre',
            )
...
class TraitementInline(admin.TabularInline):
    model = Traitement
    extra = 1
    fields = ('medicament', 'posologie', 'duree', 'commentaire', 'date_prescription')
    readonly_fields = ('date_prescription',)
    show_change_link = True


@admin.register(Ordonnance)
class OrdonnanceAdmin(BaseAdmin):
    list_display = (
        'consultation',
        'prescripteur',
        'save_by',
    ...
    search_fields = (
        'consultation__type_consultation',
        'prescripteur',
    )
    ordering = ('-date',)
    readonly_fields = ('date',)
    ...
    fieldsets = (
        ("Informations personnelles", {
            'fields': ('consultation', 'date')
        }),
        ("Prescription", {
            'fields': ('prescripteur', 'commentaire')
        }),
    )

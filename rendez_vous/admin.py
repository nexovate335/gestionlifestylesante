...
class RendezVousAdmin(BaseAdmin):
    list_display = (
        ...
        'annee_rdv', 'heure_rdv', 'statut', 'numero_dossier','commentaire', 'acte', 'save_by', 'date', 'deleted_at'
    )
    list_filter = ('mois_rdv', 'annee_rdv', 'statut', 'deleted_at')
    search_fields = ('nom_docteur', 'nom_personne', 'numero_dossier')

    actions = BaseAdmin.actions + ['restaurer_rendezvous']

    @admin.action(description="Restaurer les rendez-vous supprimés")
    def restaurer_rendezvous(self, request, queryset):
        restaurés = queryset.filter(deleted_at__isnull=False).update(deleted_at=None)
        self.message_user(request, f"{restaurés} rendez-vous restauré(s) avec succès.")

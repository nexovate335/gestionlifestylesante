...
class VaccinAdmin(BaseAdmin):
    list_display = ('patient','nom_personne','numero_dossier', 'type_vaccin', 'montant', 'save_by', 'date')
    ...
    actions = BaseAdmin.actions + ['restaurer_vaccins']

    @admin.action(description="Restaurer les vaccins supprimés")
    def restaurer_vaccins(self, request, queryset):
        restaurés = queryset.filter(deleted_at__isnull=False).update(deleted_at=None)
        self.message_user(request, f"{restaurés} vaccin(s) restauré(s) avec succès.")

...
class ViolAdmin(BaseAdmin):
    list_display = ('patient','nom_personne', 'montant', 'medecin', 'date')
    ...
    fieldsets = (
        ...
        ("Détails du viol", {
            'fields': ('montant', 'medecin', 'observation', 'commentaire', 'date')
        }),
    )

    actions = BaseAdmin.actions + ['restaurer_violences']

    @admin.action(description="Restaurer les cas supprimés")
    def restaurer_violences(self, request, queryset):
        restaurés = queryset.filter(deleted_at__isnull=False).update(deleted_at=None)
        self.message_user(request, f"{restaurés} cas de violence(s) restauré(s) avec succès.")
